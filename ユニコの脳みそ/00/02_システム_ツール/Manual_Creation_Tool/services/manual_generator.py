...
def generate_hearing_questions(input_text, preset_info=None):
    """
    【Hearing Bot (Meta-Prompt Version)】
    入力された情報と「マニュアルごとの必須要素」を比較し、
    足りない情報（Gap）を埋めるための質問を生成する。
    """
    ...
    if preset_info:
        # Meta-Prompt Logic
        prompt = f"""
        あなたはマニュアル作成の専門コンサルタントです。
        ユーザーから提供されたメモを元に、「{preset_info['name']}」を作成しようとしています。
        
        【作成するマニュアルの定義】
        特徴: {preset_info['focus']}
        必須セクション: {', '.join(preset_info['required_sections'])}
        ...
        【依頼】
        上記の入力メモと必須セクションを比較し、
        「マニュアル完成のために決定的に不足している情報」を3つ特定し、質問してください。
        
        (例: SOPなのに「判断基準」がない場合 -> "手順ごとのOK/NG判断基準はどう設定しますか？")
...
def generate_html_content(selected_option, final_text, mode, cast_key="Standard"):
    ...
    # Go one level up for templates, since this file is now in services/
...
def generate_thumbnail_html(final_text):
    ...
    サムネイル用HTMLを生成する
    """
    # Logic delegated to render_thumbnail_from_data which handles templates correctly
...
def generate_final_content(selected_option, input_text, mode):
    ...
    system_prompts = {
        "Manual": (
            "あなたは「業務資産構築のプロフェッショナル（Asset Architect）」です。\n"
            "あなたの役割は、単なる文章作成ではなく、「業務の仕組み化」と「判断基準の言語化」です。\n"
            "以下の【ライティングの鉄則 (Vol.52)】を遵守して執筆してください。\n\n"
            "1. **構造化 (Structure)**: 全てを「結論(Point)→理由(Reason)→手順」のPREP法で書くこと。\n"
            "2. **共感 (Empathy)**: 読者（新人）の心中を察し、「ここで迷うはずだ」と先回りして注釈を入れること。\n"
            "3. **二人称 (You)**: 主語を「私」ではなく「あなた」にし、読者に直接語りかけるトーンで書くこと。\n"
            "4. **資産性**: 読んだ瞬間から、教育コストがゼロになるレベルまで具体化すること。\n"
            "Markdown形式で、見出し(#)と箇条書き(-)を駆使して出力してください。"
        ),
        "Flowchart": "あなたは「業務フロー可視化の達人」です。Mermaid記法（graph TD）を使い、複雑な分岐を直感的な図にしてください。「判断菱形」を多用し、Yes/Noの分岐を明確に描くこと。",
        "FAQ": (
            "あなたは「世界一親切なメンター」です。\n"
            "ユーザーの不安（Anxiety）を解消するためのQ&Aを作成してください。\n"
            "単なる回答ではなく、「なぜその質問が出るのか？」という背景に共感（Affinity）を示してから回答すること。"
        ),
        "Slide": "あなたは「プロのプレゼンター」です。スライド構成を作成してください。聴衆の注意を引く「フック」と「要点」を短くまとめてください。"
    }
